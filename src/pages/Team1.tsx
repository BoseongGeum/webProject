    ...
    return (
        <div className="flex flex-col h-screen w-full overflow-hidden relative">
            ...
            {blocks.map((block, index) => (
                <motion.div
                    key={block.id}
                    className="flex-1 font-bold relative group bg-cover bg-center cursor-pointer"
                ...
                >
                    ...
                    {/* 컨텐츠 영역 */}
                    <div className="relative z-20 flex w-full h-full items-center justify-between px-10">
                        {/* 텍스트 영역 */}
                        <div className="w-2/3 flex items-center justify-center">
                            <AnimatePresence>
                                {activeBlock === block.id && (
                                    <motion.div
                                        key={`text-${block.id}`}
                                        className="whitespace-pre-line text-yellow-300 text-center text-2xl leading-relaxed"
                                        initial={{ opacity: 0 }}
                                        animate={{ opacity: 1 }}
                                        exit={{ opacity: 0 }}
                                ...
                                {activeBlock === block.id && (
                                    <>
                                        <motion.a
                                            ...
                                            className="w-40 px-5 py-1 border-2 border-yellow-300 bg-black bg-opacity-30 text-yellow-300 rounded-none text-xl text-center flex items-center justify-center"
                                            initial={{ opacity: 0, y: 10 }}
                                            animate={{ opacity: 1, y: 0 }}
                                            exit={{ opacity: 0, y: 10 }}
